...
def testing(model, args, dataset, device):
    ...
    with torch.no_grad():
        ...
        for batch_users in tools.mini(test_users, batch_size=int(args.test_batch_size)):
        ...
        for batch_users in tools.mini(test_users, batch_size=int(args.test_batch_size)):
            ...
            for i, u in enumerate(batch_users):
                ...
                if len(train_items) == 0:
                    continue
                exclude_users.extend([i] * len(train_items))
...
def sparsity_test(dataset, args, model, device):
    ...
    with torch.no_grad():
        for users in dataset.split_test_dict:
            ...
            for batch_users in tools.mini(users, batch_size=int(args.test_batch_size)):
                exclude_users, exclude_items = [], []
...
def item_sparsity_test(dataset, args, model, device):
    item_sparsity_results = []
    for split_dict in dataset.split_test_dict:
        item_sparsity_results.append(testing_group(model, args, dataset, device, split_dict))
    return item_sparsity_results
